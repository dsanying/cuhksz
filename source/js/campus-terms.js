  ...
  fetch(app.dataset.source)
    ...
    .then((data) => {
      ...
      app.innerHTML = `
        ...
        <div class="campus-terms-toolbar"><label><input class="campus-terms-search" type="search" placeholder="搜索缩写、英文或中文，例如 CGPA、Library、图书馆" autocomplete="off"></label><span class="campus-terms-count"></span></div>
        ...
        <p class="campus-terms-source">术语收录参考 <a href="${escapeHtml(data.source)}" target="_blank" rel="noopener">校园新生指南公开数据</a>；本站仅展示英文与中文对照。</p>`

      const search = app.querySelector('.campus-terms-search')
      ...
      const render = () => {
        ...
        grid.innerHTML = result.length ? result.map((term) => `<article class="campus-term"><span class="campus-term__english">${escapeHtml(term.english)}</span>${term.fullName ? `<span class="campus-term__full">${escapeHtml(term.fullName)}</span>` : ''}<span class="campus-term__chinese">${escapeHtml(term.chinese)}</span></article>`).join('') : '<p class="campus-terms-empty">未找到匹配术语，请换一个关键词。</p>'
      }

      search.addEventListener('input', () => { query = normalise(search.value); render() })
